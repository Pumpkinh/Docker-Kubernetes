...
READINESS_MARKER = Path("/tmp/course-app-not-ready")
...
@app.route("/internal/readiness/fail", methods=["POST"])
def fail_readiness():
    READINESS_MARKER.write_text("forced readiness failure\n", encoding="utf-8")
    return {"status": "not_ready"}, 200


@app.route("/internal/readiness/ok", methods=["POST"])
...
def index():
    ...
    response = make_response(
        render_template(
            ...
            app_banner=os.getenv("APP_BANNER", "Lesson 08: Ingress, probes and service discovery"),
            app_version=os.getenv("APP_VERSION", "lesson-08"),
